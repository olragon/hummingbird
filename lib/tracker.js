...
Tracker.prototype.attach = function (server, trackerPath) {
  // Attach to an existing server
  var oldListeners = server.listeners('request');
  server.removeAllListeners('request');

  server.on('request', function(req, res) {
    if(req.url.indexOf(trackerPath) > -1) {
...
Tracker.prototype.listen = function (server, address) {
  console.log(server);
...
Tracker.prototype.listenUdp = function (port, address) {
